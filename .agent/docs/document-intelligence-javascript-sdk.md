...
title: Azure AI Document Intelligence client library for JavaScript | Microsoft Learn
canonicalUrl: https://learn.microsoft.com/en-us/javascript/api/overview/azure/ai-form-recognizer-readme?view=azure-node-latest
...
locale: en-us
document_id: 51d2c053-3959-0715-23a8-790ee40953df
document_version_independent_id: 8f81c2f1-503c-7bb5-24af-c7a5cc2d6d89
updated_at: 2025-05-13T11:07:00.0000000Z
original_content_git_url: https://github.com/MicrosoftDocs/azure-docs-sdk-node/blob/live/docs-ref-services/latest/ai-form-recognizer-readme.md
gitcommit: https://github.com/MicrosoftDocs/azure-docs-sdk-node/blob/249b1315b308697c622f77c1d48a787bb6be64e0/docs-ref-services/latest/ai-form-recognizer-readme.md
git_commit_id: 249b1315b308697c622f77c1d48a787bb6be64e0
...
word_count: 4296
asset_id: api/overview/azure/ai-form-recognizer-readme
moniker_range_name: dc3f00e56d73e509b7ade314d9c7f100
monikers:
- azure-node-latest
item_type: Content
source_path: docs-ref-services/latest/ai-form-recognizer-readme.md
cmProducts:
...
platformId: 4fbfa79a-01c9-885e-dcae-968734f74aec
---

# Azure AI Document Intelligence client library for JavaScript | Microsoft Learn
...
[Source code](https://github.com/Azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/) | [Package (NPM)](https://www.npmjs.com/package/@azure/ai-form-recognizer) | [API reference documentation](/en-us/javascript/api/@azure/ai-form-recognizer) | [Product documentation](/en-us/azure/cognitive-services/form-recognizer/) | [Samples](https://github.com/Azure/azure-sdk-for-js/tree/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples)
...
- A [Cognitive Services or Form Recognizer resource](/en-us/azure/cognitive-services/cognitive-services-apis-create-account?tabs=multiservice,windows). If you need to create the resource, you can use the [Azure Portal](https://portal.azure.com) or [Azure CLI](/en-us/cli/azure).
...
Document Intelligence supports both [multi-service and single-service access](/en-us/azure/cognitive-services/cognitive-services-apis-create-account?tabs=multiservice,windows). Create a Cognitive Services resource if you plan to access multiple cognitive services under a single endpoint/key. For Form Recognizer access only, create a Form Recognizer resource.

You can create the resource using

**Option 1:**[Azure Portal](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesFormRecognizer)

**Option 2:**[Azure CLI](/en-us/azure/cognitive-services/cognitive-services-apis-create-account-cli?tabs=windows).
...
You can find the endpoint for your Form Recognizer resource either in the [Azure Portal](https://portal.azure.com) or by using the [Azure CLI](/en-us/cli/azure) snippet below:
...
Use the [Azure Portal](https://portal.azure.com) to browse to your Form Recognizer resource and retrieve an API key, or use the [Azure CLI](/en-us/cli/azure) snippet below:
...
API key authorization is used in most of the examples, but you can also authenticate the client with Azure Active Directory using the [Azure Identity library](https://github.com/Azure/azure-sdk-for-js/tree/@azure/ai-form-recognizer_5.1.0/sdk/identity/identity). To use the [DefaultAzureCredential](https://github.com/Azure/azure-sdk-for-js/tree/@azure/ai-form-recognizer_5.1.0/sdk/identity/identity#defaultazurecredential) provider shown below or other credential providers provided with the Azure SDK, please install the `@azure/identity` package:

```bash
npm install @azure/identity
```

To authenticate using a service principal, you will also need to [register an AAD application](/en-us/azure/cognitive-services/authentication#assign-a-role-to-a-service-principal) and grant access to the service by assigning the `"Cognitive Services User"` role to your service principal (note: other roles such as `"Owner"` will not grant the necessary permissions, only `"Cognitive Services User"` will suffice to run the examples and the sample code).
...
- `beginAnalyzeDocument`, which extracts data from an input document file stream using a custom or prebuilt model given by its model ID. For information about the prebuilt models supported in all resources and their model IDs/outputs, please see [the service's documentation of the models](https://aka.ms/azsdk/formrecognizer/models).
...
Please note that models can also be created using the Document Intelligence service's graphical user interface: [Document Intelligence Studio](https://formrecognizer.appliedai.azure.com/studio).

Sample code snippets that illustrate the use of `DocumentModelAdministrationClient` to build a model can be found below, in the "Build a Model" example section..
...
The `beginAnalyzeDocument` method can extract fields and table data from documents. Analysis may use either a custom model, trained with your own data, or a prebuilt model provided by the service (see *Use Prebuilt Models* below). A custom model is tailored to your own documents, so it should only be used with documents of the same structure as one of the document types in the model (there may be multiple, such as in a composed model).
...
The `beginAnalyzeDocument` method also supports extracting fields from certain types of common documents such as receipts, invoices, business cards, identity documents, and more using prebuilt models provided by the Document Intelligence service. The prebuilt models may be provided either as model ID strings (the same as custom document models—see the *other prebuilt models* section below) or using a `DocumentModel` object. When using a `DocumentModel`, the Document Intelligence SDK for JavaScript provides a much stronger TypeScript type for the resulting extracted documents based on the model's schema, and it will be converted to use JavaScript naming conventions.

Example `DocumentModel` objects for the current service API version (`2022-08-31`) can be found in [the `prebuilt` samples directory](https://github.com/azure/azure-sdk-for-js/tree/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/). In the following example, we'll use the `PrebuiltReceiptModel` from the [`prebuilt-receipt.ts`] file in that directory.
...
- Receipts, using [`PrebuiltReceiptModel`](https://github.com/Azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/prebuilt-receipt.ts) (as above) or the prebuilt receipt model ID `"prebuilt-receipt"`.
- Business cards, using [`PrebuiltBusinessCardModel`](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/prebuilt-businessCard.ts) or its model ID `"prebuilt-businessCard"`.
- Invoices, using [`PrebuiltInvoiceModel`](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/prebuilt-invoice.ts) or its model ID `"prebuilt-invoice"`.
- Identity Documents (such as driver licenses and passports), using [`PrebuiltIdDocumentModel`](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/prebuilt-idDocument.ts) or its model ID `"prebuilt-idDocument"`.
- W2 Tax Forms (United States), using [`PrebuiltTaxUsW2Model`](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples-dev/prebuilt/prebuilt-tax.us.w2.ts) or its model ID `"prebuilt-tax.us.w2"`.
...
```
 ...
 Custom classifiers are built in a similar way using the `beginBuildDocumentClassifier` method rather than `beginBuildDocumentModel`. Please see the [build classifier sample](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/samples/v5/typescript/src/buildClassifier.ts) for more information about building a custom classifier, as the input training data are provided in a slightly different format. For information about building a training data set for a custom classifier, see [the Document Intelligence service documentation](https://aka.ms/azsdk/formrecognizer/buildclassifiermodel).
...
For assistance with troubleshooting, see the [troubleshooting guide](https://github.com/azure/azure-sdk-for-js/blob/@azure/ai-form-recognizer_5.1.0/sdk/formrecognizer/ai-form-recognizer/TROUBLESHOOTING.md).
...
- [azure-node-legacy](https://learn.microsoft.com/en-us/javascript/api/overview/azure/ai-form-recognizer-readme?view=azure-node-legacy&accept=text/markdown)
